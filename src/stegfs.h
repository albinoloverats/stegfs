...
typedef struct stegfs_file_system_t
{
    ...
    void /*list_t*/  *files;  /*!< Optional pointer to list of known files */
}
stegfs_t;
 ...
 */
extern void stegfs_init(const char * const restrict fs, const bool c) __attribute__((nonnull(1)));
 ...
 * blocks
 */
extern void stegfs_info(stegfs_t * const restrict info) __attribute__((nonnull(1)));
 ...
 * inode
 */
extern uint64_t stegfs_stat(stegfs_file_t *, stegfs_block_t *);
extern int64_t stegfs_kill(stegfs_file_t *);
/*
 * load/save return values < 0 for an error; this value can then directly be used
 * as the return value from the fuse functions
 */
extern int64_t stegfs_load(stegfs_file_t *);
extern int64_t stegfs_save(stegfs_file_t *);
